...
public abstract class BaseControlHandler
{
    ...
    public async Task<ControlResponse> ProcessControlRequestAsync(ControlRequest request)
    {
        try
        {
            DlnaPluginLog.Debug(Logger, "Control request. Headers: {@Headers}", request.Headers);

            var response = await ProcessControlRequestInternalAsync(request).ConfigureAwait(false);
            DlnaPluginLog.Debug(Logger, "Control response. Headers: {@Headers}\n{Xml}", response.Headers, response.Xml);
    ...
    private async Task<ControlResponse> ProcessControlRequestInternalAsync(ControlRequest request)
    {
        ...
        DlnaPluginLog.Debug(Logger, "Received control request {LocalName}, params: {@Headers}", requestInfo.LocalName, requestInfo.Headers);
