...
public class Device : IDisposable
{
    ...
    public void Start()
    {
        DlnaPluginLog.Debug(_logger,"Dlna Device.Start");
    ...
    private async Task<bool> SetMute(bool mute, CancellationToken cancellationToken)
    {
        ...
        DlnaPluginLog.Debug(_logger,"Setting mute");
        var value = mute ? 1 : 0;
    ...
    public async Task SetAvTransport(string url, string? header, string metaData, CancellationToken cancellationToken)
    {
        ...
        DlnaPluginLog.Debug(_logger,"{0} - SetAvTransport Uri: {1} DlnaHeaders: {2}", Properties.Name, url, header);
    ...
    public async Task SetNextAvTransport(string url, string? header, string metaData, CancellationToken cancellationToken = default)
    {
        ...
        DlnaPluginLog.Debug(_logger,"{PropertyName} - SetNextAvTransport Uri: {Url} DlnaHeaders: {Header}", Properties.Name, url, header);
    ...
    private async void TimerCallback(object? sender)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (_connectFailureCount >= 3)
            {
                ...
                {
                    DlnaPluginLog.Debug(_logger,"Disposing device due to loss of connection");
                    action();
                    return;
                }
            }
    ...
    private async Task<TransportCommands?> GetRenderingProtocolAsync(CancellationToken cancellationToken)
    {
        ...
        var httpClient = new DlnaHttpClient(_logger, _httpClientFactory);
        DlnaPluginLog.Debug(_logger,"Dlna Device.GetRenderingProtocolAsync");
        var document = await httpClient.GetDataAsync(url, cancellationToken).ConfigureAwait(false);
        if (document is null)
        {
            return null;
