...
public class PlayToSession : ISessionController, IDisposable
{
    ...
    {
        DlnaPluginLog.Debug(_logger,"{0} - Received PlayRequest: {1}", _session.DeviceName, command.PlayCommand);
        ...
        {
            DlnaPluginLog.Debug(_logger,"{DeviceName} - Play command resulted in no items", _session.DeviceName);
        ...
        DlnaPluginLog.Debug(_logger,"{0} - Playlist created", _session.DeviceName);
    ...
    private async Task<bool> PlayItems(IEnumerable<PlaylistItem> items, CancellationToken cancellationToken = default)
    {
        ...
        DlnaPluginLog.Debug(_logger,"{0} - Playing {1} items", _session.DeviceName, _playlist.Count);
